...
"""Dataset management for axelera.app.

Dataset configuration loading, status checking, downloading and verification,
plus the calibration data loaders. Self-contained: no dependency on axelera.zoo.
"""
...
class DatasetConfig:
    def __init__(self, yaml_path: str | Path, dataset_yaml_name: DatasetYamlFile):
        self.config = self._load_yaml(yaml_path)
        self._validate_config(dataset_yaml_name)

    def _load_yaml(self, yaml_path: str | Path) -> dict:
        import yaml
                ...
                for split, split_data in data['splits'].items():
                    for item in split_data:
                        ...
                        if not item['url'].startswith(('http://', 'https://', 's3://')):
                            raise ValueError(
                                f"Invalid URL '{item['url']}' in {dataset}.{split}. "
                                "Must start with 'https://' or 's3://'."
                            )
    ...
    def get_dataset_info(self, dataset: str) -> dict | None:
    ...
    def get_description(self, dataset: str) -> str | None:
    ...
    def get_download_hint(self, dataset: str, split: str = 'val') -> str | None:
        dataset_info = self.get_dataset_info(dataset)
    ...
    def get_splits(self, dataset: str) -> list[str] | None:
        dataset_info = self.get_dataset_info(dataset)
        return list(dataset_info['splits'].keys()) if dataset_info else None

    def get_files(self, dataset: str, split: str) -> list[dict] | None:
        dataset_info = self.get_dataset_info(dataset)
    ...
    def get_urls(self, dataset: str, split: str) -> list[dict] | None:
        dataset_info = self.get_dataset_info(dataset)
        if dataset_info and split in dataset_info.get('splits', {}):
...
def _check_dataset_status(
    ...
    is_private: bool,  # noqa: ARG001 -- kept for API compatibility
    s3_available: str = '0',
    ...
    if dataset_name == 'ImageNet' and s3_available == '0':
        ...
        for file_info in files:
            ...
            if config_md5 and stored_md5 and config_md5 != stored_md5:
                return (
                    ...
                    f"MD5 mismatch for {file_info['name']}. "
                    "File may have been modified or config updated.",
                )

        update_count = len(history)
        return (
            DatasetStatus.COMPLETE,
            f"All files present and MD5 checksums match the configuration. "
            f"Last updated: {last_updated}. Total updates: {update_count}",
        )
...
def _download_dataset(dataset_dir: Path, dataset_name: str, split: str, config: DatasetConfig):
    ...
    for file_info in files:
        ...
        target_dir = dataset_dir / sub_dir if sub_dir else dataset_dir
        ...
        if dataset_name == 'ImageNet':
            ...
            if file_info['name'] in ['ILSVRC2012_img_val.tar', 'ILSVRC2012_img_train.tar']:
                file_path = target_dir.parent / file_info['name']
...
def _check_and_download_dataset(
    ...
    *,
    ...
    """Check dataset status and download if needed.

    Args:
        dataset_name: Name of the dataset.
        data_root_dir: Root directory for dataset storage.
        split: Dataset split (default: 'val').
        is_private: Whether this is a private (licensed) dataset.
        framework_dir: Path to framework root (for locating dataset YAML configs).
            Required when using YAML-based dataset configs.
        s3_available: S3 availability flag ('0'=no, '1'=yes, 'subset'=CI mode).
    ...
    if framework_dir is None:
        raise ValueError(
            "framework_dir is required for YAML-based dataset config resolution. "
            "Pass the framework root directory."
        )

    url_config = framework_dir / f'ax_datasets/{dataset_yaml_name.value}'
    if not url_config.exists():
        if is_private:
            prompt = framework_dir / f'ax_datasets/{DatasetYamlFile.DATASET_PROMPT.value}'
            if 'Customer.' in dataset_name:
                LOG.warning(
                    f"Dataset '{dataset_name}' is a customer dataset. "
                    "Please prepare your own dataset yaml file."
                )
                return
            elif prompt.exists():
                url_config = prompt
                dataset_yaml_name = DatasetYamlFile.DATASET_PROMPT
                LOG.debug(f"Using {DatasetYamlFile.DATASET_PROMPT.value}")
            else:
                raise FileNotFoundError(
                    f"Neither {dataset_yaml_name.value} nor "
                    f"{DatasetYamlFile.DATASET_PROMPT.value} found in {url_config.parent}"
                )
        else:
    ...
    if s3_available.lower() == 'subset' and 'subset' in dataset_info.get('splits', {}):
        split = 'subset'

    dataset_status, status_message = _check_dataset_status(
        data_root_dir, dataset_name, split, config, is_private, s3_available
    )
...
def check_dataset_directory(dataset_root: Path) -> tuple[bool, str]:
    """Check if the dataset directory exists and is a valid dataset directory."""
    text = []
    text.append(f'\n        Note: {dataset_root} :')
...
def _determine_drop_dirs(zip_file: Path) -> int | None:
    with zipfile.ZipFile(zip_file, 'r') as zip_ref:
        ...
        top_level_dirs = {file.split('/')[0] for file in file_list if '/' in file}
...
def download_repr_dataset(
    repr_imgs_path: Path, repr_imgs_url: str, repr_imgs_md5: str = ''
) -> bool:
    """Download and extract a representative dataset for calibration.

    Return True if downloaded and extracted, False if already present.
    """
    ...
    if not repr_imgs_path.is_dir() or not any(
        ...
        if drop_dirs is None:
            LOG.warning(
                f"Unable to determine the correct directory structure. "
                f"Please manually organize the representative dataset in {repr_imgs_path}"
            )
        else:
...
# Historical name kept for app/llm dataset-download callbacks.
download_repr_dataset_impl = download_repr_dataset
...
def check_and_download_dataset(
    ...
    Injects ``framework_dir`` and ``s3_available`` from the app environment and
    delegates to the dataset-core ``_check_and_download_dataset``.
    """
    return _check_and_download_dataset(
        dataset_name,
        data_root_dir,
        split,
        is_private,
        framework_dir=env.framework,
        s3_available=env.s3_available,
    )
