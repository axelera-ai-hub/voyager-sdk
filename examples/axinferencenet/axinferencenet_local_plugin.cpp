...
parse_args(int argc, char **argv)
{
  ...
  if (model_properties.empty() || input.empty()) {
    std::cerr
        ...
        << "Users receive the raw tensor outputs and are responsible for implementing their own \n"
        << "postprocessing (such as NMS and other steps).\n"
        << "\n"
...
void render_detections(const std::vector<Detection> &detections,
    cv::Mat &buffer, const std::vector<std::string> &labels);

class CustomDecoder : public Ax::Decode
{
  public:
  CustomDecoder(Ax::BlockingQueue<std::vector<Detection>> &detection_queue,
      int model_input_width, int model_input_height, bool letterboxed,
      float prob_threshold = 0.25f, float nms_threshold = 0.45f, int top_k = 100)
      : detection_queue_(detection_queue),
        model_input_width_(model_input_width),
        model_input_height_(model_input_height),
        letterboxed_(letterboxed),
  ...
  {
    static const Ax::StringSet props = {
      "prob_threshold",
      "nms_threshold",
      "top_k",
    };
  ...
  {
    // Handle dynamic properties if needed
    if (options.find("prob_threshold") != options.end()) {
      prob_threshold_ = std::stof(options.at("prob_threshold"));
    }
    if (options.find("nms_threshold") != options.end()) {
      nms_threshold_ = std::stof(options.at("nms_threshold"));
    }
    if (options.find("top_k") != options.end()) {
      top_k_ = std::stoi(options.at("top_k"));
    }
  }

  void decode_to_meta(const AxTensorsInterface &tensors,
      unsigned int /*subframe_index*/, unsigned int /*number_of_subframes*/,
      Ax::MetaMap & /*map*/, const AxDataInterface &video) override
  {
    ...
    auto pvideo = std::get_if<AxVideoInterface>(&video);
    if (!pvideo) {
      throw std::runtime_error("Video data interface is not of type AxVideoInterface");
    }

    const auto *data = static_cast<const float *>(tensors[0].data);
    const auto img_width = pvideo->info.width;
    const auto img_height = pvideo->info.height;

    auto dims = tensors[0].sizes;
    while (!dims.empty() && dims.front() == 1) {
      dims.erase(dims.begin());
    }
    if (dims.size() != 2) {
      throw std::runtime_error("Expected output tensor to have 2 dimensions with shape (classes+4, N), got shape: "
                               + Ax::Internal::join(dims, ", "));
    }
    const auto num_channels = static_cast<int>(dims[0]);
    const auto num_anchors = static_cast<int>(dims[1]);
    const auto num_classes = num_channels - 4;
    ...
    float scale = std::min(float(model_input_width_) / img_width,
        float(model_input_height_) / img_height);
    float new_unpad_w = img_width * scale;
    float new_unpad_h = img_height * scale;
    float pad_w = (model_input_width_ - new_unpad_w) / 2.0f;
    float pad_h = (model_input_height_ - new_unpad_h) / 2.0f;
    ...
    for (int i = 0; i < num_anchors; ++i) {
      ...
      if (score > prob_threshold_) {
        ...
        if (letterboxed_) {
          ...
          // Simple resize
          x0 = x0 * img_width / model_input_width_;
          y0 = y0 * img_height / model_input_height_;
          x1 = x1 * img_width / model_input_width_;
          y1 = y1 * img_height / model_input_height_;
        }
    ...
    cv::dnn::NMSBoxes(bboxes, scores, prob_threshold_, nms_threshold_, indices, 1.f, top_k_);
    for (int idx : indices) {
      Detection det;
      det.bbox = bboxes[idx];
    ...
    detection_queue_.push(std::move(detections));
  }

  private:
  Ax::BlockingQueue<std::vector<Detection>> &detection_queue_;
...
main(int argc, char **argv)
{
  ...
  const bool letterboxed = true;

  // We use BlockingQueue to communicate between the frame_completed callback
  // and the main loop and between the custom decoder and the main loop
  Ax::BlockingQueue<std::shared_ptr<Frame>> ready;
  Ax::BlockingQueue<std::vector<Detection>> detection_queue;

  // read the axnet pipeline and find the decode plugin, remove it
  auto props = Ax::read_inferencenet_properties(model_properties, logger);
  auto out_plugin = std::find_if(
      std::begin(props.postproc), std::end(props.postproc), [](const auto &p) {
        return p.lib.find("decode_to_raw_tensor") != std::string::npos;
      });
  if (out_plugin == std::end(props.postproc)) {
    throw std::runtime_error("Error: Could not find decode plugin in postprocessing pipeline.");
  }
  *out_plugin = Ax::OperatorProperties{};

  // Load all the plugins, and push our own decoder onto the end of the postproc
  auto loaded = Ax::load_inferencenet_plugins(props, logger, nullptr);
  loaded.postproc.emplace_back(std::make_unique<CustomDecoder>(
      detection_queue, model_input_width, model_input_height, letterboxed));

  auto net = Ax::create_inference_net(
      std::move(loaded), logger, Ax::forward_to(ready), {}, nullptr);
  ...
  while (1) {
    ...
    auto detections = detection_queue.wait_one();
