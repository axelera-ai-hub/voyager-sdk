...
Auto-generated from rt_demo.py's @publicdemo `detection` by
tools/export_pipeline_builder_demo.py.  Edit the original demo in rt_demo.py
rather than this file -- regenerate with
`python tools/export_pipeline_builder_demo.py detection`.

Usage:
    python detection.py PATH [--display opencv|console|none|auto] [--no-wait]
"""
...
def main(args):
    with display.App(renderer=args.display) as visualizer:
        vis = visualizer.create_window('detection', (args.window_width, args.window_height))
        pipeline = op.seq(
