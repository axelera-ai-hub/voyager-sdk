...
OCSort::OCSort(float det_thresh_, int max_age_, int min_hits_, float iou_threshold_,
    int delta_t_, std::string asso_func_, float inertia_, bool use_byte_, int max_id_)
{
  max_age = max_age_;
  min_hits = min_hits_;
...
OCSort::update(Eigen::MatrixXf dets)
{
  frame_count += 1;
  ...
  std::vector<int> map_dets_second_to_dets;
  for (int i = 0; i < output_results.rows(); i++) {
    if (true == inds_second(i)) {
      ...
      map_dets_second_to_dets.push_back(i);
    }
  ...
  if (true == use_byte && dets_second.rows() > 0 && unmatched_trks.size() > 0) {
    ...
    if (iou_left.maxCoeff() > iou_threshold) {
      ...
      for (auto m : matched_indices) {
        ...
        trackers[trk_ind].update(
            &tmp_box, dets_second(det_ind, 5), map_dets_second_to_dets[det_ind]);
  ...
  for (int i : unmatched_dets) {
    ...
    trk.latest_detection_id = map_dets_first_to_dets[i];
  ...
  // print out existing id with class_id
  // for (const auto &tracker : trackers) {
  //   std::cout << "id" << tracker.id << "(" << tracker.cls << ") ";
  // }
  // std::cout << std::endl;
