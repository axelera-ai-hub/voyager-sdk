...
Eigen::VectorXf
k_previous_obs(std::unordered_map<int, Eigen::VectorXf> observations_, int cur_age, int k)
{
  ...
  auto iter = std::max_element(observations_.begin(), observations_.end(),
      [](const std::pair<int, Eigen::VectorXf> &p1, const std::pair<int, Eigen::VectorXf> &p2) {
